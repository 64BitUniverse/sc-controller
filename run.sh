...
function rebuild_c_modules() {
	...
	LIB=$( python -c 'import platform ; print("lib.linux-%s-%s.%s" % ((platform.machine(),) + platform.python_version_tuple()[0:2]))' )
...
for cmod in ${C_MODULES[@]}; do
	...
	reported_version=$(PYTHONPATH="." python -c 'import os, ctypes; lib=ctypes.CDLL("./'lib${cmod}'.so"); print(lib.'${cmod}'_module_version())')
