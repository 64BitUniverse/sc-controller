		...
		if message.startswith("Profile:"):
			with self.lock:
				try:
					...
					log.exception(e)
					# Originally tb = unicode(exc).encode("utf-8").encode('string_escape') 
					tb = str(exc).encode('unicode_escape')
			...
			with self.lock:
				try:
					...
					tb = str(traceback.format_exc()).encode('unicode_escape')
					client.wfile.write(b"Fail: " + tb + b"\n")
					return
